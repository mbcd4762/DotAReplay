...
// String을 ASCII로 변환하는 함수
std::string ConvertToASCII(const String& str)
{
    std::string result;
...
// String을 std::wstring으로 변환하는 함수
std::wstring ConvertToWString(const String& str)
{
    std::string temp(str.c_str()); // String을 std::string으로 변환
    std::wstring wstr(temp.begin(), temp.end()); // std::string을 std::wstring으로 변환
    return wstr;
}
...
void FolderWindow::rebuild()
{
  ...
  for (int i = 0; i < found.length(); i++)
  {
    ...
    else
    {
      ...
      if (colShow[colName])
      {
          std::wstring wGameName = ConvertToWString(fitem.gameName);
          std::string utf8GameName = ConvertToUTF8(wGameName);
