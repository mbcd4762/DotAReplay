...
std::string GetDocumentsPath()
{
    ...
    {
        return std::string(path) + "\\Warcraft III\\Replay";
    }
    return "";
}
...
MainWnd::MainWnd()
{
  ...
  String documentsPath = GetDocumentsPath().c_str();

  //replayTree = new ReplayTree(cfg.replayPath, this);
  //replayTree = new ReplayTree(cfg.replayPath, this);
  replayTree = new ReplayTree(documentsPath, this);
  replayTree = new ReplayTree(documentsPath, this);
...
//  pushView(new FolderViewItem(cfg.replayPath));
  pushView(new FolderViewItem(documentsPath));
  ...
  getFileInfo(String::buildFullName(documentsPath, "LastReplay.w3g"), fi);
  ...
  case WM_UPDATEPATH:
    {
      ...
      if (getFileInfo(String::buildFullName(GetDocumentsPath().c_str(), "LastReplay.w3g"), fi))
        lastReplayTime = fi.ftime;
      else
        lastReplayTime = 0;
      //replayTree->setPath(cfg.replayPath);
      replayTree->setPath(GetDocumentsPath().c_str());
    }
    return 0;
  case WM_UPDATEFILE:
    ...
    {
      FileInfo fi;
      //if (getFileInfo(String::buildFullName(cfg.replayPath, "LastReplay.w3g"), fi) && fi.ftime != lastReplayTime)
      if (getFileInfo(String::buildFullName(GetDocumentsPath().c_str(), "LastReplay.w3g"), fi) && fi.ftime != lastReplayTime)
      {
        if (cfg.autoView)
        {
        ...
        if (cfg.autoCopy)
        {
          ...
          if (cache)
          {
            ...
            String path = cache->format(cfg.copyFormat, GetDocumentsPath().c_str());
            //path = String::fixPath(String::buildFullName(cfg.replayPath, path));
            path = String::fixPath(String::buildFullName(GetDocumentsPath().c_str(), path));
