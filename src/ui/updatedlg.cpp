  ...
  if (versionInfo)
  {
    ...
    try {
      if (VerQueryValue(versionInfo, "\\VarFileInfo\\Translation", &buf, &vLen) && vLen == 4) {
        entry.printf("%04X%04X\\", *(int*)buf & 0xFFFF,
                     (*(int*)buf >> 16) & 0xFFFF);
      } else {
        entry.printf("%04X04B0\\", GetUserDefaultLangID());
      }

      //if (VerQueryValue(versionInfo, (entry + "ProductVersion").c_str(), &buf, &vLen)) {
      //  if (buf != NULL) {
      //    thisVersion = parseVersion((char*)buf);
      //    lastVersion = thisVersion;
      //  } else {
      //    // Handle the case where buf is NULL
      //    throw std::runtime_error("ProductVersion buffer is NULL");
      //  }
      //} else {
      //  // Handle the case where VerQueryValue fails
      //  throw std::runtime_error("VerQueryValue for ProductVersion failed");
      //}
    } catch (const std::exception& e) {
      // Log or handle the error
      // For example, you might log the error message e.what() to a file or
      // display it
