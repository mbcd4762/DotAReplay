...
void W3GReplay::addMessageUnicode(int type, int id, uint32 time, wchar_t const* fmt, ...)
{
    W3GMessage& msg = messages.push();
    msg.id = id;
    msg.mode = CHAT_NOTIFY;
    ...
    vsprintf(msg.text.getBuffer(), fmt, ap);
    msg.text.setLength(len);*/
    int len = _vscwprintf(fmt, ap);  // 유니코드에 맞게 변경
    msg.text.resize(len * sizeof(wchar_t));  // 버퍼 크기 조정
    vswprintf((wchar_t*)msg.text.getBuffer(), len + 1, fmt, ap);  // 유니코드에 맞게 변경
    msg.text.setLength(len * sizeof(wchar_t));
    va_end(ap);
}
