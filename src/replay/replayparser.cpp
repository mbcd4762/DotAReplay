    ...
    while (!replay->eof())
    {
        ...
        case 0x17:
        {
            ...
            if (leaves == numPlayers)
                ...
                addMessage(CHAT_NOTIFY_LEAVER, id, state.time, u8"%s 게임을 나갔습니다.",
                    players[id]->format_Simple());
            if (reason == 0x01)
            {
                switch (result)
                {
    ...
    while (replay->tell() < end)
    {
        ...
        while (replay->tell() < next)
        {
            ...
            case 0x01:
                state.pause = true;
                if (!quickLoad)
                    addMessage(CHAT_NOTIFY_PAUSE, id, state.time, u8"%s 게임을 일시 중지했습니다.", player->format_Simple());
                break;
                // Resume game
            case 0x02:
                state.pause = false;
                if (!quickLoad)
                    addMessage(CHAT_NOTIFY_PAUSE, id, state.time, u8"%s 게임을 재개했습니다.", player->format_Simple());
                break;
                // Save game
            case 0x06:
                if (!quickLoad)
                    addMessage(CHAT_NOTIFY_PAUSE, id, state.time, u8"%s has saved the game to %s.",
            ...
            case 0x32:
                if (!quickLoad)
                    addMessage(0, id, state.time, u8"%s uses a cheat %s", player->format_Simple(), parser->name().cut(7));
                //addMessage(0, id, state.time, u8"");
                break;
                // Change ally options
            case 0x50:
            {
                ...
                if (!quickLoad)
                {
                    ...
                    if (share && !player->share[slot])
                        addMessage(CHAT_NOTIFY_CONTROL, id, state.time, u8"%s -> %s 유닛 컨트롤을 공유합니다.",
                            player->format_Simple(), other->format_Simple());
                    else
                        addMessage(CHAT_NOTIFY_CONTROL, id, state.time, u8"%s -> %s 유닛 컨트롤 공유를 비활성화 합니다.",
                            player->format_Simple(), other->format_Simple());
                }
            ...
            case 0x51:
                if (!quickLoad)
                {
                    ...
                    W3GPlayer* other = getPlayerInSlot(slot);
                    if (other)
                        addMessage(0, id, state.time, u8"%s 이(가) %s 에게 금 %d 과 나무 %d 을 보냅니다.",
                            player->format_Simple(), other->format_Simple(), gold, lumber);
                }
                break;
